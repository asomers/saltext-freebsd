...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/freebsd/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/freebsd/*")
            ...
            #    "--include=src/saltext/freebsd/*,tests/*"
            # )
        finally:
